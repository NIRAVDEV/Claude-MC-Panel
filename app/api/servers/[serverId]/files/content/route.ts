...
export async function GET(
  ...
  try {
    ...
    // Call Wings agent to get file content
    const response = await sendWingsRequest(
      server.node.ip,
      server.node.port,
      server.node.verificationToken,
      `/files/content?serverName=${encodeURIComponent(server.name)}&userEmail=${encodeURIComponent(user.email || 'user@example.com')}&path=${encodeURIComponent(path)}`
    )
...
export async function POST(
  ...
  try {
    ...
    // Call Wings agent to save file content
    const response = await sendWingsRequest(
      server.node.ip,
      server.node.port,
      server.node.verificationToken,
